...
metadata:
  source_repo: https://github.com/bermudas/toscacloud_cli
  version: "0.1.0"
---
...
Scripts live next to this `SKILL.md`, in `scripts/`:
...
After installation by `qavajs-skill`'s `bin/init.mjs`, this skill ends up at
`<TARGET>/skills/browser-verify/` (e.g. `.claude/skills/browser-verify/`).
The scripts you call are at `<SKILL_ROOT>/scripts/`. Construct the absolute
path before running:
